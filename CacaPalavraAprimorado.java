...
public class CacaPalavraAprimorado {

  private CacaPalavraAprimorado() {
  ...
  private void esquerdaParaDireita (
    ...
    for (lin = 0; lin < mapaQtdLinha; lin++) {
      for (col = 0; col < mapaQtdColuna; col++) {
         ...
         if (col > (mapaQtdColuna - palavras[palavraAtual][0].length()) && letraAtual == 0) {
          ...
          if (letraAtual == palavras[palavraAtual][0].length()) {
            palavras[palavraAtual][1] =
              "[" + lin + "," + (col - palavras[palavraAtual][0].length() + 1) + "]";
            letraAtual = 0;
            break;
          }
        } else {
  ...
  String[][] palavras) {
    ...
    for (lin = 0; lin < mapaQtdLinha; lin++) {
      for (col = (mapaQtdColuna - 1); col >= 0; col -= 1) {
        //melhorar desempenho, dar break quando não precisa mais procurar
    ...
    for (col = 0; col < mapaQtdColuna; col++) {
      for (lin = 0; lin < mapaQtdLinha; lin++) {
         ...
         if (lin > (mapaQtdLinha - palavras[palavraAtual][0].length()) && letraAtual == 0) {
          ...
          if (letraAtual == palavras[palavraAtual][0].length()) {
            palavras[palavraAtual][1] =
              "[" + (lin - palavras[palavraAtual][0].length() + 1) + "," + col + "]";
            letraAtual = 0;
            break;
          }
        } else {
  ...
  private void baixoParaCima ( int palavraAtual,
  int lin,
  int col,
  int letraAtual,
  int mapaQtdLinha,
  ...
  String[][] palavras) {
      ...
      for (col = 0; col < mapaQtdColuna; col++) {
        for (lin = (mapaQtdLinha - 1); lin >= 0; lin -= 1) {
          //melhorar desempenho, dar break quando não precisa mais procurar
  ...
  private void diagonalEsquerdaAcima(
    int palavraAtual,
    int lin,
    int col,
    int letraAtual,
    ...
    for (lin = 0; lin < mapaQtdLinha; lin++) {
      for (col = 0; col < mapaQtdColuna; col++) {
        if (
          lin < (palavras[palavraAtual][0].length() - 1) &&
          ...
          if (
            (mapa[lin][col] == palavras[palavraAtual][0].charAt(letraAtual))
          ) {
            letraAtual += 1;
            for (
              auxiliar = 1;
              auxiliar < palavras[palavraAtual][0].length();
              auxiliar += 1
            ) {
              if (
                mapa[lin - auxiliar][col - auxiliar] ==
                palavras[palavraAtual][0].charAt(letraAtual)
              ) {
                letraAtual += 1;
                if (letraAtual == palavras[palavraAtual][0].length()) {
  ...
  private void diagonalDireitaAcima(
    int palavraAtual,
    int lin,
    int col,
    int letraAtual,
    ...
    for (lin = 0; lin < mapaQtdLinha; lin++) {
      for (col = 0; col < mapaQtdColuna; col++) {
        if (
          lin < (palavras[palavraAtual][0].length() - 1) &&
          col > (mapaQtdColuna - palavras[palavraAtual][0].length())
        ) {
          break;
        } else if (
          lin >= (palavras[palavraAtual][0].length() - 1) &&
  ...
  private void diagonalEsquerdaAbaixo(
    int palavraAtual,
    int lin,
    int col,
    int letraAtual,
    ...
    for (lin = 0; lin < mapaQtdLinha; lin++) {
      for (col = 0; col < mapaQtdColuna; col++) {
        if (
          lin > (mapaQtdLinha - palavras[palavraAtual][0].length()) &&
          col < (palavras[palavraAtual][0].length() - 1)
        ) {
          break;
        } else if (
  ...
  private void diagonalDireitaAbaixo(
    int palavraAtual,
    int lin,
    int col,
    int letraAtual,
  ...
  public void mapaPesquisar(
    ...
    for (int palavraAtual = 0; palavraAtual < palavras.length; palavraAtual++) {
      ...
      esquerdaParaDireita(palavraAtual, lin, col, letraAtual, mapaQtdLinha, mapaQtdColuna, mapa, palavras);
      }
      if (palavras[palavraAtual][1] == null) {
      direitaParaEsquerda(palavraAtual, lin, col, letraAtual, mapaQtdLinha, mapaQtdColuna, mapa, palavras);
    }
      if (palavras[palavraAtual][1] == null) { 
      cimaParaBaixo(palavraAtual, lin, col, letraAtual, mapaQtdLinha, mapaQtdColuna, mapa, palavras);
    }
      if (palavras[palavraAtual][1] == null) {
      baixoParaCima(palavraAtual, lin, col, letraAtual, mapaQtdLinha, mapaQtdColuna, mapa, palavras);
    }
      if (palavras[palavraAtual][1] == null) {
      diagonalEsquerdaAcima(palavraAtual,lin,col,letraAtual,mapaQtdLinha,mapaQtdColuna,mapa,palavras);
    }
      if (palavras[palavraAtual][1] == null) {
      diagonalDireitaAcima(palavraAtual,lin,col,letraAtual,mapaQtdLinha,mapaQtdColuna,mapa,palavras);
    }
      if (palavras[palavraAtual][1] == null) {
      diagonalEsquerdaAbaixo(palavraAtual,lin,col,letraAtual,mapaQtdLinha,mapaQtdColuna,mapa,palavras);
    }
      if (palavras[palavraAtual][1] == null) {
      diagonalDireitaAbaixo(palavraAtual,lin,col,letraAtual,mapaQtdLinha,mapaQtdColuna,mapa,palavras);
    }
    }
  }
